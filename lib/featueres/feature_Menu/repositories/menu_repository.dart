...
import 'package:poortak/featueres/feature_Menu/data/models/faq_model.dart';
...
class MenuRepository {
  ...
  Future<DataState<ContactUsInfo>> getContactUsInfo() async {
    try {
      final response = await apiProvider.callGetContactUsInfo();
      final data = response.data;

      if ((response.statusCode == 200 || response.statusCode == 201) &&
          data is Map<String, dynamic> &&
          data['ok'] == true) {
        final parsed = ContactUsResponse.fromJson(data);
        return DataSuccess(ContactUsInfo.fromItems(parsed.data));
      }

      if (data is Map<String, dynamic>) {
        return DataFailed(
          data['message']?.toString() ?? "خطا در دریافت اطلاعات تماس",
        );
      }

      return const DataFailed("خطا در دریافت اطلاعات تماس");
    } on DioException catch (e) {
      final responseData = e.response?.data;
      if (responseData is Map<String, dynamic>) {
        final message = responseData['message']?.toString();
